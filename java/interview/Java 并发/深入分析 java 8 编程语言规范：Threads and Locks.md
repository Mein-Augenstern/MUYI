...
**关于JVM中对象模型的内容也阅读链接**：https://github.com/Mein-Augenstern/MUYI/blob/master/java/interview/JVM/JVM%E5%86%85%E5%AD%98%E7%AE%A1%E7%90%86/Java%E5%AF%B9%E8%B1%A1%E6%A8%A1%E5%9E%8B.md
...
>
> 当多个线程尝试访问同一个同步块时，如果出现了竞争，JVM会首先尝试使用轻量级锁定，这是通过在Mark Word中采用CAS（Compare-and-Swap）操作来实现的。如果轻量级锁定失败且线程竞争激烈，JVM会膨胀锁，将其升级为重量级锁定。在这种情况下，锁的信息就会从轻量级锁定在线程栈上的>
> 锁记录转变为一个在堆中的监视器对象，并且Mark Word会被更新为指向该监视器。
>
...
>
> 这个过程在可视化上可能类似于：
<img width="755" alt="截屏2023-12-31 20 11 12" src="https://github.com/Mein-Augenstern/MUYI/assets/34135120/d5beabc2-4a09-4657-8636-f5ea07527dcf">
...
**补充知识点：注意上面文字中关联描述，在JVM中对象模型中，不同的JVM实现中关于对象模型是如何关联监视器以及等待集合的具体实现也是不同的。**
...
1.线程 t 会加入到对象 m 的**等待集合**中，执行 **加锁编号 n 对应的解锁操作**
  ...
  > 方法thread1开始执行时，线程尝试获得object的监视器锁。只有在获取到锁之后，同步块中的代码才能被执行。

  > 一旦线程获得了object的监视器锁，它接下来会调用object.wait()方法。调用wait()会导致几件事情发生：

  > 当前线程会释放object对象上的监视器锁。这意味着其他线程现在可以竞争并获取这个监视器锁。

  > 当前线程会进入object的等待集合（wait set），在那里它将停止执行，直到它被notify()或notifyAll()方法唤醒，或者被中断。

  > 如果当前线程在调用wait()时持有其他监视器锁（如锁定了另一个对象），调用wait()不会释放这些其他的监视器锁。只有object上的监视器锁被释放。

  > 当object.notify()或object.notifyAll()被另一个线程调用时（且这个调用发生在持有object监视器锁的同步块中），或者当前线程被中断，等待中的线程（包括调用wait()的线程）将被唤醒。如果线程是因为notify()或> notifyAll()被唤醒的，则它必须重新获得object的监视器锁才能继续执行。如果存在多个等待线程，它们会竞争同一个监视器锁。

  > 当线程重新获得监视器锁后，它会从wait()调用之后的代码继续执行。

  > 如果在等待时线程被中断，它会抛出InterruptedException。在这段代码中，中断异常被捕获了，但异常处理块中并没有具体的代码，只是一个注释，提示在这里可以执行一些异常处理的代码。

  > 当同步块执行完成后（无论是正常完成还是因异常退出），线程将释放object上的监视器锁，使得其他线程可以尝试获取该锁。

  > 代码注释中特别强调了在wait()调用期间，仅释放了调用wait()方法的对象（在本例中是object）的监视器锁，而没有影响到其他可能由同一个线程持有的监视器锁。这是Java多线程同步机制中的一个重要细节。
...
**这一节主要在讲线程进出等待集合的各种情况，同时，最好要知道中断是怎么用的，中断的状态重置发生于什么时候。这里的 1，2，3，4 的发生顺序非常关键，大家可以仔细再看看是不是完全理解了，之后的几个小节还会更具体地阐述这个，参考代码请看 17.2.4 小节我写的简单的例子。**
